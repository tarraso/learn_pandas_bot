...
**⚠️ Важно**: Если впервые выбираете VPS, сначала прочитайте **[Как выбрать VPS - KVM vs OpenVZ](docs/CHOOSING_VPS.md)**
...
⚠️ **CRITICAL**: Выбор типа виртуализации!

#### Как выбрать правильный план:

**На сайте AlphaVPS планы разделены по категориям:**

1. **Зайдите на**: https://alphavps.com/vps
2. **Ищите планы с пометкой "KVM"**:
   - ✅ "KVM SSD VPS"
   - ✅ "KVM NVMe VPS"
   - ✅ Любой план где явно написано "KVM"

3. **ИЗБЕГАЙТЕ планов**:
   - ❌ "OpenVZ VPS"
   - ❌ "Container VPS"
   - ❌ Если не указан тип - уточните в support!

#### Визуальные отличия в каталоге:

**KVM планы** (нужные нам):
```
┌─────────────────────────────┐
│ KVM SSD VPS - 1GB          │  ← Смотрите на название!
│ 1 vCPU, 1GB RAM, 15GB SSD  │
│ KVM Virtualization         │  ← Должно быть явно указано
│ $5/month                    │
└─────────────────────────────┘
```

**OpenVZ планы** (НЕ подходят):
```
┌─────────────────────────────┐
│ Container VPS - 1GB        │  ← "Container" = OpenVZ
│ 1 vCPU, 1GB RAM, 15GB SSD  │
│ OpenVZ/LXC                 │  ← Docker НЕ заработает!
│ $3/month                    │
└─────────────────────────────┘
```
...
```
1. AlphaVPS Website → Products → VPS Hosting
2. Выберите категорию: "KVM VPS" (не Container/OpenVZ)
3. Выберите план по RAM (1GB, 2GB, 4GB...)
4. Configure:
   - OS: Ubuntu 22.04 LTS (рекомендуется)
   - Location: Ближайшая к пользователям
     • EU: Bulgaria, Germany, UK
     • US: New York, Los Angeles
   - Control Panel: None (не нужна)
5. Checkout
```

#### Если не уверены:

**Перед оплатой проверьте в корзине:**
- Должно быть написано "KVM" в названии плана
- Или в описании: "Full virtualization"

**Если не указано:**
- Откройте чат поддержки на сайте
- Спросите: "Is this plan KVM or OpenVZ?"
- Если OpenVZ - выберите другой план!

#### После получения VPS - обязательно проверьте:

```bash
# Подключитесь по SSH
ssh root@YOUR_VPS_IP

# Проверьте тип виртуализации
systemd-detect-virt
```

**Ожидаемый результат:**
```
kvm        ← ✅ Отлично! Docker заработает
```

**Если показывает:**
```
openvz     ← ❌ ПРОБЛЕМА! Docker не заработает полноценно
lxc        ← ❌ ПРОБЛЕМА! Это тоже container-based
```

**Что делать если получили OpenVZ:**
1. Свяжитесь с поддержкой AlphaVPS
2. Попросите перенести на KVM план
3. Или запросите refund и закажите заново

#### Почему это важно:

| Виртуализация | Docker | Производительность | Цена |
|---------------|--------|-------------------|------|
| **KVM** | ✅ Полная поддержка | Хорошая | Дороже |
| **OpenVZ** | ⚠️ Ограниченная | Отличная | Дешевле |

**OpenVZ проблемы с Docker:**
- Нельзя загрузить свой kernel
- Ограничения на mount операции
- Проблемы с overlay filesystem
- Контейнеры могут не запускаться
- Непредсказуемое поведение

**Итог**: Для Docker нужен **только KVM**! Не экономьте $1-2, иначе ничего не заработает.
